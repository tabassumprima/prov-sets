...
class DiscountRateService
{
    ...
    public function __construct()
    {
        ...
        $this->model = new DiscountRate();
    }
    public function create($request)
    {
        $request->merge(['organization_id' => $this->organizationService->getAuthOrganizationId()]);
        $request->merge(['status_id'       => CustomHelper::fetchStatus('started')]);
    ...
    {
        $discount        = $this->fetch($id);
        $organization_id = $this->organizationService->getAuthOrganizationId();
        $fileName        = CustomHelper::generateUniqueName($request->discount_file);

        //merge path
        $request->merge(["path" => $fileName]);
        $discount->files()->create($request->all());

        $storagePath = CustomHelper::fetchOrganizationStorage($organization_id, 'provision_files.discount_rates');
        //storing file temporary. File will be deleted after validaton
        $path = Storage::disk("private")->putFileAs($storagePath, $request->discount_file, $fileName);

        // Storing file to s3
        if ($this->validateCsv($path, $organization_id))
            Storage::disk('s3')->putFileAs($storagePath, $request->discount_file, $fileName);
    }
    ...
    public function validateCsv($filePath, $organization_id)
    {
        ...
        $rulePath         = CustomHelper::fetchOrganizationStorage($organization_id, 'provision_rules.discount_rates');
        $validationHelper = $csv->validateCSV($path, ',', $rulePath.'/rule.json', $invalid_path);
        Storage::delete($path);                       //delete local file
        return $validationHelper;
    }

    public function getLatestDiscountRate()
    {
