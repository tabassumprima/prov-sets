...
use App\Helpers\{CsvValidation, CustomHelper};
...
class RiskAdjustmentService
{
    ...
    public function createFile($id, $request)
    {
        ...
        $fileName = CustomHelper::generateUniqueName($request->risk_adjustment_file);
        //merge path
        $request->merge(["path" => $fileName]);
        $data->files()->create($request->all());

        $storagePath = CustomHelper::fetchOrganizationStorage($organization_id, 'provision_files.risk_adjustments');

        //storing file temporary. File will be deleted after validaton
        $path = Storage::disk("private")->putFileAs($storagePath, $request->risk_adjustment_file, $fileName);

        //Storing file to s3
        if ($this->validateCsv($path, $organization_id))
            Storage::disk('s3')->putFileAs($storagePath, $request->risk_adjustment_file, $fileName);
    }
    ...
    public function validateCsv($filePath, $organization_id)
    {
        ...
        $rulePath         = CustomHelper::fetchOrganizationStorage($organization_id, 'provision_rules.risk_adjustments');
        $validationHelper = $csv->validateCSV($path, ',', $rulePath.'/rule.json', $invalid_path);
        Storage::delete($path);                       //delete local file
        return $validationHelper;
    }

    public function getLatestRiskAdjustment()
    {
