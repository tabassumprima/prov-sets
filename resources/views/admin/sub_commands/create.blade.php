    ...
    <div class="content-wrapper">
        ...
        <div class="content-body">
            ...
            <section class="form-control-repeater">
                <form action="{{ route('lambda-sub-functions.store') }}" method="post">
                    @csrf
                    <div class="row">
                        <!-- Invoice repeater -->
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-12 col-md-6">
                                            <div class="form-group position-relative">
                                                ...
                                                <select class="select2 form-control glcode" name="lambda_function_id">
                                                    ...
                                                    <option value={{ $lambda->id }}>{{$lambda->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        <div class="col-12 col-md-6">
                                            <div class="form-group position-relative">
                                                <label for="voucher-date" class="form-label required">Command</label>
                                                <select id="command" name="command" class="select2 form-control">
                                                    @if (isset($subCommands) && !empty($subCommands))
                                                    @foreach ($subCommands->lambda_sub_function_commands as $command)
                                                    <option value="{{ $command->name }}"
                                                        {{ old('command') == $command->name ? 'selected' : '' }}>
                                                                {{ $command->name }}</option>
                                                    @endforeach
                                                    @endif
                                                </select>
                                        ...
                                        <div class="col-12 col-md-6">
                                            <div class="form-group position-relative">
                                                <label for="voucher-date">Narration</label>
                                                <input type="text" id="narration" name="narration" value="{{ old('narrations') }}" class="form-control" />
                                            </div>
                                        </div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="outer-repeater">
                                                ...
                                                <div class="row d-flex align-items-end">
                                                    <div class="col-md-4 col-sm-12 col-lg-4 col-xl-4">
                                                        <div class="form-group">
                                                            ...
                                                            <select class="select2 form-control glcode" name="gl_code_id">
                                                                <option disabled selected>Select GlCode</option>

                                                                @foreach ($glcodes as $glCode)
                                                                <option value={{ $glCode->id }}>{{$glCode->code }} - {{ $glCode->description }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="col-md-2 col-sm-12 col-lg-2 col-xl-2">
                                                        <div class="d-flex align-items-center mb-1">
                                                            <input type="radio" name="reverse_opening" class="reverse-opening">
                                                            <label class="reverse-lable">Reverse Opening Balance</label>
                                                        </div>
                                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-6">
                                                <button class="btn btn-icon btn-outline-primary" type="button" data-repeater-create id="repeater-button">
...
<script>
    $(document).ready(function() {
        ...
        // Handle Select2 selections
        $(document).on('select2:select', '.glcode', function() {
            let level = $(this).closest('.col-md-4').next().find('.level');
            let level_value = level.find('option:first-child').val();
            level.val(level_value).trigger('change');
        });

        $(document).on('select2:select', '.level', function() {
            let glcode = $(this).closest('.col-md-4').prev().find('.glcode');
            let glcode_value = glcode.find('option:first-child').val();
            glcode.val(glcode_value).trigger('change');
        });

        // Ensure Select2 is reinitialized when a new row is added
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        repeaterContainer.addEventListener('click', function(event) {
            ...
            if (clickedCheckbox.type === 'radio' && clickedCheckbox.classList.contains('reverse-opening')) {
