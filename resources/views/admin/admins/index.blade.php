                                ...
                                @foreach ($users as $user)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="custom-control custom-control-success custom-checkbox">
                                                <input type="checkbox" name="user_is_active" class="custom-control-input"
                                                    ...
                                                    {{ $user->email == Auth::user()->email ? 'disabled' : '' }}  />
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('users.impersonate', ['user' => CustomHelper::encode($user->id)]) }}" class="btn btn-warning btn-sm">Login</a>
                                            @endif
                                            <a id="user_edit:{{ $user->id }}"
                                                href="{{ route('admin-users.edit', ['admin_user' => CustomHelper::encode($user->id)]) }}"><i
                                                    data-feather="edit"></i></a>
                                            @if( $user->email != Auth::user()->email ? 'disabled' : '' )
                                            <a data-route="{{ route('admin-users.destroy', [CustomHelper::encode($user->id)]) }}"
        ...
        $(document).ready(function() {
            ...
            $('input[name="user_is_active"]:checkbox').change(
                function() {
                    ...
                    $.ajax({
                        url: "/admin/admin-users/" + id + '/status-update',
                        type: "POST",
                        data: {
                            id: id,
                            type: type,
