                            ...
                            <div class="tab-content">
                                ...
                                <div class="tab-pane active" id="account" aria-labelledby="account-tab" role="tabpanel">
                                    ...
                                    <form class="form-validate" method="POST"
                                        action="{{ route('admin-users.update', [CustomHelper::encode($user->id)]) }}"
                                        autocomplete="off">
                                        @csrf
                                        @method('PUT')
                                        <div class="row">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#genKey").on('click', function() {
                $.ajax({
                    url: "{{ route('users.admin.generate2fa', [CustomHelper::encode($user->id)]) }}",
                    type: "POST",
                    data: {
            ...
            this.disabled = true;
            $.ajax({
                url: '{{route("users.admin.welcome-mail", CustomHelper::encode($user->id))}}',
                type: 'POST',
                data: {
                    '_token': '{{csrf_token()}}',
                },
                success: function(data) {
                    $(".alert").css({'display':'block'}).addClass('alert-success');
                    $(".alert > .alert-body").html(data.message);
                },
                error: function(data, status, error) {
                    $(".alert").css({'display':'block'}).addClass('alert-danger');
                    $(".alert > .alert-body").html('Error: '+error);
        ...
        $("#reset-mail").click(function(){
            this.disabled = true;
            $.ajax({
                url: '{{route("users.admin.reset-mail", CustomHelper::encode($user->id))}}',
                type: 'POST',
                data: {
                    '_token': '{{csrf_token()}}',
                },
